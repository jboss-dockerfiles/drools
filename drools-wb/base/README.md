...
If you don't want to extend this image and you just want to try Drools Workbench, please take a look at the `jboss/drools-workbench-showcase:6.5.0.Final` Docker image, it contains some default configurations.                       
...
To run a container:
    ...
    docker run -p 8080:8080 -p 8001:8001 -d --name drools-workbench jboss/drools-workbench:6.5.0.Final
...
If you are looking for a Drools Workbench image that does not require to add custom configurations, try our `jboss/drools-workbench-showcase:6.5.0.Final` Docker image.
...
In order to extend this image, your Dockerfile must inherit from:
    ...
    FROM jboss/drools-workbench:6.5.0.Final
...
You can find an example by looking at the Dockerfile for `jboss/drools-workbench-showcase:6.5.0.Final` image.
...
To spin up a shell in one of the containers try:
    ...
    docker run -p 8080:8080 -p 8001:8001 -d --name drools-workbench jboss/drools-workbench:6.5.0.Final /bin/bash
